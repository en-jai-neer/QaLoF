...
from utils.test_config import generate_configs
...
class ModelArguments:
    ...
    sp_config_path: Optional[str] = field(
        default=None,   
        metadata={
            "help": (
                "The path to the switch precision config file"
...
def main():
    ...
    model = GPT2ForQuestionAnsweringCustom.from_pretrained(model_args.model_name_or_path)
    ...
    # new - load sp_config
    sp_config = None
    if model_args.sp_config_path is not None:
        print("Loading sp_config from", model_args.sp_config_path)
        with open(model_args.sp_config_path, "r") as f:
